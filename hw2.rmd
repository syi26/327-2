...
monte.carlo.pi = function(n, draw=FALSE) {
  ...
  if (draw == TRUE) {
    ...
    lines(x = cos(seq(from = 0, to = 2 * pi, by = 0.001)), y = sin(seq(from = 0, to = 2 * pi, by = 0.001)))
  }
  return(4 * count / n)
}
...
```{r}
pi_estimate_100 <- replicate(500, expr = monte.carlo.pi(100))
pi_estimate_1000 <- replicate(500, expr = monte.carlo.pi(1000))
layout(matrix(c(1, 2), nrow = 1, ncol = 2))
plot(density(pi_estimate_100), xlim = c(2.5, 3.5), main = "pi estimates for n = 100")
plot(density(pi_estimate_1000), xlim = c(2.5, 3.5), main = "pi estimates for n = 1000")
```
...
scores$Grade <- mapply(scores$Pct.gr, scores$Pctile.gr, FUN = CourseGrade, USE.NAMES = FALSE)
print(scores)
```
...
```{r}
round(tapply(scores$E1, scores$Grade, FUN = mean), digits = 1)
```
